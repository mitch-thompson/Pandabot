...
func TestActionTriggeringBasedOnPartyState(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		for j := 0; j < memberCount; j++ {
			...
			// Avoid duplicate names in the same test iteration
			if members[name] {
				j--
				continue
			}
		...
		for _, action := range actions {
			if action.Type != "cure" && action.Type != "na_spell" && action.Type != "manual_spell" {
	...
	for i := 0; i < 100; i++ {
		...
		for _, hp := range testHPs {
			...
			if member.NeedsHealing {
				...
				// But compare based on HP threshold categories
				if hp < 50 && lastPriority > 0 && priority < lastPriority {
					t.Errorf("Iteration %d: Lower HP (%d) should have higher priority than %d (previous was HP %d)",
						i, hp, lastPriority, lastHP)
				}
