...
func DefaultCastingConfig() *CastingConfig {
	return &CastingConfig{
		...
		MPReservation: 50, // Keep 50 MP in reserve
...
func (ce *CastingEngine) RequestCast(request *CastRequest) error {
	log.Printf("[QUEUE DEBUG] New cast request received: %s -> %s (Type: %s, Priority: %d)", 
		request.ID, request.SpellName, ce.castTypeToString(request.Type), request.Priority)
	...
	if err := ce.validateRequest(request); err != nil {
		log.Printf("[QUEUE DEBUG] Cast request validation failed: %s -> %v", request.ID, err)
	...
	if err := ce.resolveSpellSelection(activeCast); err != nil {
		log.Printf("[QUEUE DEBUG] Spell selection failed for %s: %v", request.ID, err)
		return fmt.Errorf("spell selection failed: %v", err)
	}
	
	log.Printf("[QUEUE DEBUG] Spell selection completed for %s: %s (Type: %s)", 
		request.ID, activeCast.Request.SpellName, ce.castTypeToString(activeCast.Request.Type))
...
func (ce *CastingEngine) selectOptimalCure(context *CastContext) (*cureSelector.CureOption, error) {
	...
	if context.TargetEntity != nil && context.TargetEntity.HPPercent < 30 {
		...
		log.Printf("[CURE DEBUG] Efficiency mode: target HP %d%% >= 30%%", 
			func() uint8 { if context.TargetEntity != nil { return context.TargetEntity.HPPercent } else { return 100 } }())
	}
...
func (ce *CastingEngine) processCast(activeCast *ActiveCast) {
	log.Printf("[QUEUE DEBUG] Starting cast processing: %s -> %s", activeCast.Request.ID, activeCast.Request.SpellName)
...
func (ce *CastingEngine) logQueueState(operation string, requestID string) {
	queueSize := len(ce.activeCasts)
	log.Printf("[QUEUE DEBUG] %s - RequestID: %s | Queue Size: %d", operation, requestID, queueSize)
	
	if queueSize > 0 {
		log.Printf("[QUEUE DEBUG] Active Casts:")
		for id, cast := range ce.activeCasts {
			spellInfo := cast.Request.SpellName
			if cast.Request.Type == CastTypeSequence {
				spellInfo = fmt.Sprintf("%s (seq: %d/%d - %v)", 
					cast.Request.SpellName, 
					cast.CurrentSpellIndex+1, 
					len(cast.SpellsInSequence),
					cast.SpellsInSequence)
			}
			
			log.Printf("[QUEUE DEBUG]   - %s: %s -> %s (State: %s, Priority: %d)", 
				id, 
				spellInfo, 
				cast.Request.Target, 
				ce.castStateToString(cast.State),
				cast.Request.Priority)
		}
	} else {
		log.Printf("[QUEUE DEBUG] Queue is empty")
	}
}
...
func (ce *CastingEngine) NotifySpellComplete(requestID string, success bool, errorMsg string) {
	...
	log.Printf("[QUEUE DEBUG] Received spell completion notification: %s (success: %t, error: %s)", requestID, success, errorMsg)
